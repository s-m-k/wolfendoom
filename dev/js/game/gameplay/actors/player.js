/*include:js/core/namespace.js*/
/*include:js/game/core/input.js*/
/*include:js/core/math.js*/
/*include:js/game/core/renderer.js*/
/*include:js/game/core/timer.js*/
/*include:js/game/gameplay/weapons/weapons.js*/

(function () {
    'use strict';

    var input = wd.input,
        weapons = wd.weapons,
        math = wd.math,
        renderer = wd.renderer,
        timer = wd.timer;

    wd.Player = (function () {
        function Player(config) {
            var self = this;

            config = config || {};
            this.position = config.position || { x: 0, y: 0 };
            this.velocity = { x: 0, y: 0 };
            this.angle = 0;
            this.swing = 0;
            this.velocityCompensation = 0;
            this.playerSize = 14;

            this.weapon = new weapons.Pistol();

            (function () {
                var collisionPhysics = self.collisionPhysics,
                    openSelectedDoor = self.openSelectedDoor;

                self.collisionPhysics = function () {
                    return collisionPhysics.apply(self, arguments);
                };

                self.openSelectedDoor = function () {
                    return openSelectedDoor.apply(self, arguments);
                };
            }());
        }

        Player.prototype = {
            constructor: Player,
            accelerate: function (acceleration) {
                this.velocity.x += Math.cos(this.angle) * acceleration;
                this.velocity.y += Math.sin(this.angle) * acceleration;

                this.swing += 4;
            },
            strafe: function (acceleration) {
                this.velocity.x -= Math.sin(this.angle) * acceleration;
                this.velocity.y += Math.cos(this.angle) * acceleration;

                this.swing += 4;
            },
            rotate: function (omega) {
                this.angle += omega;

                if (this.angle > Math.PI * 2) {
                    this.angle -= Math.PI * 2;
                }

                if (this.angle < 0) {
                    this.angle += Math.PI * 2;
                }
            },
            openSelectedDoor: function (x, y, normalX, normalY, length, wall) {
                if (wall.door) {
                    this.parentMap.openDoor(wall.door);
                }
            },
            openDoor: function () {
                if (this.parentMap) {
                    this.parentMap.collideWithLevel(this.position, this.playerSize + 4,
                        this.playerSize + 4, this.openSelectedDoor);
                }
            },
            navigate: function () {
                if (input.key(38)) {
                    this.accelerate(0.2);
                }

                if (input.key(40)) {
                    this.accelerate(-0.2);
                }


                if (input.key(90)) {
                    if (input.key(39)) {
                        this.strafe(0.2);
                    }

                    if (input.key(37)) {
                        this.strafe(-0.2);
                    }
                } else {
                    if (input.key(39)) {
                        this.rotate(0.04);
                    }

                    if (input.key(37)) {
                        this.rotate(-0.04);
                    }
                }

                if (input.key(88)) {
                    this.weapon.pullTrigger();
                } else {
                    this.weapon.releaseTrigger();
                }

                if (input.key(32)) {
                    this.openDoor();
                }
            },
            setParentMap: function (map) {
                this.parentMap = map;
            },
            collisionPhysics: function (x, y, normalX, normalY, length) {
                var correction;

                correction = Math.max(1, 4 * (1 - length / this.playerSize));

                this.velocityCompensation = math.dot(normalX, normalY, this.velocity.x, this.velocity.y);

                if (this.velocityCompensation < 0) {
                    this.position.x = x + normalX * this.playerSize;
                    this.position.y = y + normalY * this.playerSize;

                    this.velocity.x -= normalX * this.velocityCompensation * correction;
                    this.velocity.y -= normalY * this.velocityCompensation * correction;
                }
            },
            processCollision: function () {
                if (this.parentMap) {
                    this.parentMap.collideWithLevel(this.position, this.playerSize, this.playerSize, this.collisionPhysics);
                    this.parentMap.collideWithSprites(this.position, this.playerSize, this.playerSize + 16, this.collisionPhysics);
                }
            },
            process: function () {
                this.processCollision();
                this.navigate();

                math.addVectors2d(this.position, this.velocity);
                math.mulVec2dScalar(this.velocity, 0.94);
                this.swing *= 0.9;

                this.weapon.process();
            },
            render: function () {
                renderer.getFrustum()
                    .setAngle(this.angle)
                    .setPosition(
                        this.position.x,
                        this.position.y,
                        Math.sin(timer.getTicks() / 6) * this.swing
                    );
            },
            renderHUD: function () {
                this.weapon.render(this.swing);
            }
        };

        return Player;
    }());
}());